...
{
	vqs_t t = vqs_set(vec3_set(0.0f, 0.0f, 0.0f),
...
static inline vqs_t vqs_absolute_transform(const vqs_t* local,
					   const vqs_t* parent)
{
	if (!local || !parent) {
		return vqs_default();
	...
	// position
	vec3f_t tns =
		vec3_add(parent->position,
			 quat_rotate(p_rot_norm,
				     vec3_mul(parent->scale, local->position)));
...
static inline vqs_t vqs_relative_transform(const vqs_t* absolute,
					   const vqs_t* parent)
{
	if (!absolute || !parent) {
		return vqs_default();
	...
	// position
	vec3f_t tns =
		vec3_div(quat_rotate(p_rot_inv, vec3_sub(absolute->position,
							 parent->position)),
			 parent->scale);
...
static inline vqs_t vqs_from_mat4(const mat4f_t* m)
{
	...
	mat4_decompose(m, (float*)&translation, (float*)&rotation,
		       (float*)&scale);
	return vqs_set(translation,
		       quat_from_euler(rotation.x, rotation.y, rotation.z),
		       scale);
}

static inline vec3f_t vqs_forward(const vqs_t* transform)
{
	return (quat_rotate(transform->rotation, vec3_set(0.0f, 0.0f, -1.0f)));
}

static inline vec3f_t vqs_backward(const vqs_t* transform)
{
	return (quat_rotate(transform->rotation, vec3_set(0.0f, 0.0f, 1.0f)));
}

static inline vec3f_t vqs_left(const vqs_t* transform)
{
	return (quat_rotate(transform->rotation, vec3_set(-1.0f, 0.0f, 0.0f)));
}

static inline vec3f_t vqs_right(const vqs_t* transform)
{
	return (quat_rotate(transform->rotation, vec3_set(1.0f, 0.0f, 0.0f)));
}

static inline vec3f_t vqs_up(const vqs_t* transform)
{
	return (quat_rotate(transform->rotation, vec3_set(0.0f, 1.0f, 0.0f)));
}

static inline vec3f_t vqs_down(const vqs_t* transform)
{
	return (quat_rotate(transform->rotation, vec3_set(0.0f, -1.0f, 0.0f)));
}

static inline void vqs_reset(vqs_t* transform)
{
