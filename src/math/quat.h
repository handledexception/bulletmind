...
{
	return quat_set(a.w * b.x + b.w * a.x + a.y * b.z - b.y * a.z,
...
{
	return quat_set(a.x * b.x + a.x * b.w + a.y * b.z - a.z * b.y,
...
static inline quat_t quat_from_to_rotation(vec3f_t src, vec3f_t dst)
{
	...
	if (d >= 1.f) {
		...
		const f32 s = sqrtf(vec3_sqrlen(src) * vec3_sqrlen(dst)) +
			      vec3_dot(src, dst);
...
static inline quat_t quat_look_rotation(vec3f_t position, vec3f_t target,
					vec3f_t up)
{
