...
{
	return smpte_get_lo_word_value(st, kDropFrameBitMask,
				       kDropFrameBitShift);
}

void smpte_set_color_frame_bit(smpte_tc_t* st, u8 val)
{
	smpte_set_lo_word_value(st, val, kColorFrameBitMask,
				kColorFrameBitShift);
}
u8 smpte_get_color_frame_bit(smpte_tc_t* st)
{
	return smpte_get_lo_word_value(st, kColorFrameBitMask,
				       kColorFrameBitShift);
}
...
{
	if (fps.numerator % 47980 == 0 || fps.numerator % 48000 == 0 ||
	    fps.numerator % 50000 == 0 || fps.numerator % 60000 == 0)
		return true;
	return false;
}
...
void frames_to_timecode(u32 frames, fps_t fps, bool drop, timecode_t* tc)
{
	...
	if (drop) {
		u32 dropped_frames =
			round_fps_rational(fps_rational * 0.066666);
		u32 dropped_frames_per_sec = frames_per_sec - dropped_frames;
		u32 dropped_frames_per_min =
			(59 * frames_per_sec) + dropped_frames_per_sec;
		u32 dropped_frames_per_ten_min =
			(9 * dropped_frames_per_min) + frames_per_min;
...
u32 timecode_to_frames(timecode_t* tc)
{
	...
	f64 fps_rational =
		(double)tc->fps.numerator / (double)tc->fps.denominator;
	...
	if (tc->drop) {
		...
		if ((tc->secs == 0) && ((tc->mins % 10) > 0) &&
		    ((tc->frame & ~1) == 0))
			frm = 2;
		frame = ((hour_frames * tc->hours) +
			 (minute_frames * tc->mins) + (tb * tc->secs) + frm) -
			(drop_frames * (total_minutes - (total_minutes / 10)));
	} else {
		frame = ((hour_frames * tc->hours) +
			 (minute_frames * tc->mins) + (tb + tc->secs) +
			 tc->frame);
	}
...
{
	if ((st->mins_units != 0) && (st->secs_units == 0) &&
	    (st->secs_tens == 0) && (st->frame_units == 0) &&
	    (st->frame_tens == 0)) {
		st->frame_units += 2;
	}
}
