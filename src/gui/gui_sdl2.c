...
#define SDL_FLAGS (SDL_INIT_VIDEO | SDL_INIT_AUDIO | \
		SDL_INIT_EVENTS | SDL_INIT_TIMER | SDL_INIT_GAMECONTROLLER)
#if defined(_WIN32)
#include <Windows.h>
#endif
...
bool gui_get_window_rect_sdl2(const gui_window_t* window, rect_t* rect, bool client);
...
bool gui_create_window_sdl2(gui_window_t* window)
{
    if (!gui || !window)
        return false;
    int flags = SDL_WINDOW_SHOWN | SDL_WINDOW_MOUSE_CAPTURE | SDL_WINDOW_RESIZABLE;
    window->data = (gui_window_data_t*)BM_ALLOC(sizeof(gui_window_data_t));
    memset(window->data, 0, sizeof(gui_window_data_t));
    window->data->sdl_window = SDL_CreateWindow(window->title,
...
{
    if (gui_is_valid_window(window))
        SDL_SetWindowPosition(window->data->sdl_window, rect->x, rect->y);
}
...
{
    if (gui) {
        return (bool)SDL_SetRelativeMouseMode(
...
bool gui_move_mouse_sdl2(s32 x, s32 y)
{
    if (gui && gui->mouse.window && gui->mouse.window->data) {
        SDL_WarpMouseInWindow(gui->mouse.window->data->sdl_window, x, y);
        return true;
    } else {
        return false;
    }
