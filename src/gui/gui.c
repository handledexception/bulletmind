...
gui_window_t* gui_create_window(const char* title, s32 x, s32 y, s32 w, s32 h, s32 flags, gui_window_t* parent)
{
    if (!gui) {
        if (!gui_init())
            return NULL;
    ...
    if (BM_WINDOW_POS_IS_UNDEFINED(window->x) || BM_WINDOW_POS_IS_UNDEFINED(window->y)) {
