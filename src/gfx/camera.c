...
vec3f_t  gfx_camera_screen_to_world(const camera_t* cam, vec3f_t coords, s32 view_x, s32 view_y)
{
...
mat4f_t  gfx_camera_get_proj(const camera_t* cam)
{
	...
	{
		proj_mat = mat4_persp(cam->fov, aspect, cam->near_plane, cam->far_plane);
		break;
	}
...
void gfx_camera_offset_orientation(camera_t* cam, f32 yaw, f32 pitch)
{
    quat_t x = quat_angle_axis(DEG_TO_RAD(yaw), vec3_set(0.f, 1.f, 0.f));              // Absolute up
    quat_t y = quat_angle_axis(DEG_TO_RAD(pitch), gfx_camera_right(cam));            // Relative right
    cam->transform.rotation = quat_mul(quat_mul(x, y), cam->transform.rotation);
}
